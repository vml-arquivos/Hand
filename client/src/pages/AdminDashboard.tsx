...
import { CheckCircle2, Clock3, Loader2, MessageCircle, Settings, ShieldCheck, TicketCheck, XCircle } from "lucide-react";
      ...
      <section className="container py-8">
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[1.4fr_.8fr]">
          <Card className="border-0 shadow-xl"><CardHeader><CardTitle className="flex items-center gap-2"><TicketCheck /> Pedidos</CardTitle></CardHeader><CardContent className="space-y-4">
            {pedidos.map((item) => {
              const numerosBilhetes = item.bilhetes.map((b) => String(b.numero).padStart(4, "0"));
              const mensagem = `Olá ${item.comprador.nome}! Seu pagamento da rifa foi confirmado ✅. Bilhetes: ${numerosBilhetes.join(", ") || "ainda em processamento"}. Boa sorte!`;

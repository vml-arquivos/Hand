...
import { HeartHandshake, Loader2, LockKeyhole, QrCode, Sparkles, Ticket } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#faf6ef] text-[#22180e]">
      <section className="mx-auto max-w-7xl px-4 pb-28 pt-6 md:px-6 md:pb-10 md:pt-10">
        <nav className="mb-8 flex items-center justify-between rounded-2xl border border-[#eadbc2] bg-white px-4 py-3 shadow-sm md:px-6">
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#f4dfbc] text-[#593b1f]"><HeartHandshake /></span><strong>Rifas Beneficentes</strong></div>
          <a href="/admin" className="inline-flex items-center gap-2 text-sm text-[#593b1f] hover:opacity-80"><LockKeyhole className="h-4 w-4" /> Admin</a>
        </nav>

        <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr] lg:gap-10">
          <div className="space-y-6">
            <Badge className="border-[#e7c782] bg-[#f8ebd2] text-[#7f5525] hover:bg-[#f8ebd2]"><Sparkles className="mr-1 h-3 w-3" /> Campanha ativa</Badge>
            <h1 className="text-4xl font-semibold leading-tight tracking-[-0.04em] md:text-6xl">{rifa.nome}</h1>
            <p className="max-w-2xl text-base leading-8 text-[#493624] md:text-lg">{rifa.descricao}</p>
            <div className="grid gap-4 sm:grid-cols-3">
              <Card className="border-[#ecdcc5] bg-white shadow-sm"><CardContent className="p-5"><p className="text-sm text-muted-foreground">Valor por bilhete</p><strong className="text-2xl">{moeda.format(Number(rifa.precoBilhete))}</strong></CardContent></Card>
              <Card className="border-[#ecdcc5] bg-white shadow-sm"><CardContent className="p-5"><p className="text-sm text-muted-foreground">Confirmados</p><strong className="text-2xl">{rifa.vendidos}</strong></CardContent></Card>
              <Card className="border-[#ecdcc5] bg-white shadow-sm"><CardContent className="p-5"><p className="text-sm text-muted-foreground">Disponíveis</p><strong className="text-2xl">{rifa.disponiveis}</strong></CardContent></Card>
            </div>
            {rifa.imagemUrl ? <img src={rifa.imagemUrl} alt={rifa.nome} className="h-[350px] w-full rounded-3xl object-cover shadow-[0_20px_60px_rgba(28,17,8,0.25)] md:h-[460px]" /> : null}
          </div>

          <Card className="sticky top-4 h-fit border-[#e8dbc8] bg-white shadow-[0_30px_70px_rgba(37,23,9,0.18)]">
            <CardContent className="p-6 md:p-8">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Bilhetes gerados somente após confirmação manual do pagamento pelo administrador.</p>
              ...
              <div className="mb-6 mt-5 space-y-2">
                ...
                <p className="text-xs text-muted-foreground">{rifa.vendidos} de {rifa.totalBilhetes} confirmados. {rifa.pendentes} em análise.</p>
              </div>
              ...
              <form onSubmit={onSubmit} className="space-y-4">
                ...
                <Button className="hidden h-12 w-full bg-[#2b2116] text-white hover:bg-[#4a341f] md:inline-flex" disabled={criarPedido.isPending || rifa.disponiveis <= 0}>{criarPedido.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Ticket className="mr-2 h-4 w-4" />} Finalizar pedido pendente</Button>
      ...
      <div className="fixed inset-x-0 bottom-0 border-t border-[#e6d8c1] bg-white/95 p-4 backdrop-blur md:hidden">
        ...
        }} className="h-12 w-full bg-[#2b2116] text-white" disabled={criarPedido.isPending || rifa.disponiveis <= 0}>
